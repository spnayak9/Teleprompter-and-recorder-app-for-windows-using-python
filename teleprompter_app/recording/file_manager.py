...
class RecordingFileManager:
    ...
    def prepare_session(self, project_root: Path, config: any) -> RecordingFiles:
        ...
        # Determine next numeric session index
        pattern = re.compile(r"^(?P<num>\d+)\.(wav|flac|srt|txt|mkv|mp4|mov)$", re.IGNORECASE)
        max_num = 0
        for p in list(audio_dir.iterdir()) + list(subtitles_dir.iterdir()) + list(video_dir.iterdir()):
            if not p.is_file():
                continue
        ...
        return RecordingFiles(
            ...
            flac_path=audio_dir / f"{base_name}.flac",
            video_path=video_dir / f"{base_name}.mkv",
